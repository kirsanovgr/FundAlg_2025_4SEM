...
TEST_F(BigIntTest, ModularExponentiation) {
	EXPECT_EQ(mod_exp(BigInt(2), BigInt(3), BigInt(5)), BigInt(3));
	EXPECT_EQ(mod_exp(BigInt(3), BigInt(3), BigInt(7)), BigInt(6));
}
TEST_F(BigIntTest, exp_0) {
	EXPECT_EQ(mod_exp(BigInt(5), BigInt(0), BigInt(3)), BigInt(1));
	EXPECT_EQ(mod_exp(BigInt(0), BigInt(0), BigInt(3)), BigInt(1));
	EXPECT_EQ(mod_exp(BigInt(0), BigInt(5), BigInt(7)), BigInt(0));
	EXPECT_EQ(mod_exp(BigInt(0), BigInt(0), BigInt(7)), BigInt(1));
}
TEST_F(BigIntTest, exp_1) {
	EXPECT_EQ(mod_exp(BigInt(5), BigInt(1), BigInt(3)), BigInt(2));
	EXPECT_EQ(mod_exp(BigInt(5), BigInt(3), BigInt(1)), BigInt(0));
}

TEST_F(BigIntTest, BigNumbers) {
	BigInt base("123456789");
	BigInt exp("100");
	BigInt mod("1000000007");
	BigInt result = mod_exp(base, exp, mod);
	EXPECT_EQ((std::ostringstream() << result).str(), "791765815");

	EXPECT_EQ(mod_exp(BigInt(7), BigInt(3), BigInt(5)), BigInt(3));
	EXPECT_EQ(mod_exp(BigInt(4), BigInt(4), BigInt(10)), BigInt(6));
	EXPECT_EQ(mod_exp(BigInt(3), BigInt(5), BigInt(10)), BigInt(3));
}

TEST_F(BigIntTest, VaryBigNumbers) {
	BigInt big_base("12345678901234567890");
	BigInt big_exp("123456");
	BigInt big_mod("10000000000000000000");
	BigInt big_result = mod_exp(big_base, big_exp, big_mod);
	EXPECT_EQ((std::ostringstream() << big_result).str(), "0");

	EXPECT_EQ(mod_exp(BigInt(-2), BigInt(3), BigInt(5)), BigInt(-3));
}

TEST_F(BigIntTest, ModExpDivisionByZero) {
	EXPECT_THROW(mod_exp(BigInt(2), BigInt(3), BigInt(0)), std::domain_error);
}