...
TEST_F(BigIntTest, AdditionOperations) {
	...
	EXPECT_EQ(a + BigInt("99999999999999999999"), BigInt("123456789112345678901234567889"));
}

TEST_F(BigIntTest, SubtractionOperations) {
	EXPECT_EQ(a - a, zero);
	EXPECT_EQ(b - b, zero);
	EXPECT_EQ(a - BigInt("100000000000"), BigInt("123456789012345678801234567890"));
}
...
TEST_F(BigIntTest, EdgeCaseOperations) {
	...
	EXPECT_EQ(max_val * max_val, BigInt("999999999999999999999999999998000000000000000000000000000001"));

	BigInt min_val("-999999999999999999999999999999");
	EXPECT_EQ(min_val * min_val, BigInt("999999999999999999999999999998000000000000000000000000000001"));
}
...
TEST_F(FFTTest, BigNumbersBase10) {
	BigInt tmp{num1};
	tmp.change_base(10);
