...
TEST_F(BigIntTest, StringConstructorValidation) {
    ...
    EXPECT_THROW(BigInt("aa12345"), std::invalid_argument);
...
TEST_F(BigIntTest, AdditionOperations) {
    EXPECT_EQ(BigInt("10") + BigInt("9999999999"), BigInt("10000000009"));
    EXPECT_EQ(a + zero, a);
...
TEST_F(BigIntTest, SubtractionOperations) {
    ...
    EXPECT_EQ(a - BigInt("-1000000000"), BigInt("123456789012345678902234567890"));
}
...
TEST_F(BigIntTest, DivisionOperations) {
    ...
    EXPECT_EQ(BigInt("214824242") / BigInt("12474821748241242"), BigInt("0"));
}
...
TEST_F(BigIntTest, StreamOperations) {
    std::stringstream ss;
    ss << (b + BigInt("43210"));
    EXPECT_EQ(ss.str(), "-987654321098765432109876500000");

    BigInt num;
    ss >> num;
    EXPECT_EQ(num, b + BigInt("43210"));
